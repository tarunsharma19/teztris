...
class FA2_mint(FA2_core):
    ...
    def mint(self, params):
        sp.verify((self.is_administrator(sp.sender)) | (sp.sender == self.data.crowdsale), message = self.error_message.not_admin())
...
class FA2(FA2_change_metadata, FA2_token_metadata, FA2_mint, FA2_administrator, FA2_pause, FA2_core):
    ...
    def __init__(self, config, metadata, admin , crowdsale):
        ...
        FA2_core.__init__(self, config, metadata, paused = False, administrator = admin , crowdsale = crowdsale)
...
def add_test(config, is_default = True):
    ...
    def test():
        ...
        c1 = FA2(config = config,
                 ...
                 admin = admin.address , crowdsale=sp.address("tz1-crowdsale"))
        scenario += c1
...
if "templates" not in __name__:
    ...
    sp.add_compilation_target("FA2_comp", FA2(config = environment_config(),
                              metadata = sp.utils.metadata_of_url("https://example.com"),
                              admin = sp.address("tz1NaGu7EisUCyfJpB16ktNxgSqpuMo8aSEk"), crowdsale=sp.address("tz1NaGu7EisUCyfJpB16ktNxgSqpuMo8aSEk")))